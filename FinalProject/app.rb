...
post '/uploadUsers' do
  halt(401, 'Not Authorized') unless session[:role] == 'Admin'
  #IF possible I want to delete all of the items in the current database
  #And then allow the new CSV uploaded to take it's place
  #To avoid the duplicate error
  #Vote.delete_all
...
post '/uploadWebsites' do
  ...
  begin
    ...
    file_name = params['file_source'][:filename].to_str

    system('unzip ' + file_name + ' -d ./public/files')
